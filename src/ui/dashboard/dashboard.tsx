...
function Sidebar({ notes, view, onViewChange, counts, tags }: { notes: any[]; view: string; onViewChange: (v: string) => void; counts: Record<string, number>; tags: { name: string; count: number }[]; }) {
  ...
  return (
    <div className="sidebar">
      <div className="sidebar-header">
        <pre className="ascii-logo">_      _    ___ _   _
| |    / \  |_ _| \ | |
| |   / _ \  | ||  \| |
| |__/ ___ \ | || |\  |
|___/_/   \_\___|_| \_|</pre>
      ...
      <nav className="sidebar-nav">
        ...
        <div className="nav-section">
          ...
          <div className="nav-subsection" id="statusSection">
            <Item view="status-active"><span className="status-dot status-active" /><span>Active</span><span className="nav-count" id="count-active">{String(counts.active)}</span></Item>
            <Item view="status-onhold"><span className="status-dot status-onhold" /><span>On Hold</span><span className="nav-count" id="count-onhold">{String(counts.onhold)}</span></Item>
            <Item view="status-completed"><span className="status-dot status-completed" /><span>Completed</span><span className="nav-count" id="count-completed">{String(counts.completed)}</span></Item>
            <Item view="status-dropped"><span className="status-dot status-dropped" /><span>Dropped</span><span className="nav-count" id="count-dropped">{String(counts.dropped)}</span></Item>
          </div>
        </div>
        <div className="nav-section">
          <div className="nav-section-header">
          ...
          <div className="nav-subsection" id="tagsSection">
            {tags.map(t => (
              <a href="#" className="nav-item nav-nested" onClick={(e) => { e.preventDefault(); onViewChange(`tag-${t.name}`); }}>
...
function NotesListPanel({ notes, currentNoteId, onAddNote, onSelect }: { notes: any[]; currentNoteId: number | null; onAddNote: () => void; onSelect: (n: any) => void; }) {
  return (
    <div className="notes-list-panel">
      ...
      <div className="notes-list-body" id="notesListPanel">
        {notes.length === 0 ? (
          ...
          notes.map(n => (
            <div className={`note-list-item${currentNoteId === n.id ? ' active' : ''}`} style={{ borderLeftColor: n.color || '#667eea' }} onClick={() => onSelect(n)}>
    ...
    <div>
      ...
      {open ? (
        <div style={{ display: 'flex', gap: '6px', marginTop: '6px' }}>
          {colors.map(c => (
            <div style={{ width: 18, height: 18, borderRadius: 4, background: c, border: '1px solid #ddd', cursor: 'pointer' }} onClick={() => { onChange(c); setOpen(false); }} />
          ))}
        </div>
      ) : null}
    </div>
...
function EditorPanel({ note, onChange, onDelete, onStatus, onTagAdd, onTagRemove, onColor }: { note: any | null; onChange: (n: any) => void; onDelete: (n: any) => void; onStatus: (n: any) => void; onTagAdd: (n: any) => void; onTagRemove: (n: any) => void; onColor: (n: any) => void; }) {
  if (!note) {
    return (
